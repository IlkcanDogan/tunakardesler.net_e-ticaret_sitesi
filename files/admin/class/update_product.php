<?php  
	...
	if($requestMethod == "POST"){
		...
		if($categoryId != '' && $brandId != '' && $productName != '' && $stockQuantity != '' && $price != '' && $discountType != ''){
			session_start();
			if ($_SESSION["USERNAME"] != '' && strlen($_SESSION["PASSWORD"]) == 32) {
				...
				if(true){
					$db = new Database();
					if($db->ProductUpdate($productId,$productName,$detail,$price,$stockQuantity,$discountType,$discountValue,$discountCode,$brandId,$productStatus,$categoryId,$subcategoryId,$subsubcategoryId,$imageInfo)){
