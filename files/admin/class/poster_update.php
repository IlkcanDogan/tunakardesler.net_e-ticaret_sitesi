<?php  
	...
	if($requestMethod == "POST"){
		$mainId = $func->PostData("MAIN_ID");
		$primaryId = $func->PostData("PRIMARY_ID");
		$secondaryId = $func->PostData("SECONDARY_ID");
		$productId = $func->PostData("PRODUCT_ID");
		$posterId = $func->PostData("POSTER_ID");
		...
		if(true){
			...
			if ($_SESSION["USERNAME"] != '' && strlen($_SESSION["PASSWORD"]) == 32) {
				...
				$db = new Database();
				if($db->PosterImageUpdate($fileNo,$mainId,$primaryId,$secondaryId,$productId,$posterId)){
